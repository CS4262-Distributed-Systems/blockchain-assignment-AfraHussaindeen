...
public final class AssetTransfer implements ContractInterface {
    ...
    public Asset DuplicateAsset(final Context ctx, final String assetID, final String newOwner) {
        ...
        // Check the existence of the asset with given assetID
        ChaincodeStub stub = ctx.getStub();
        String assetJSON = stub.getStringState(assetID);

        if (assetJSON == null || assetJSON.isEmpty()) {
            String errorMessage = String.format("Asset with ID %s does not exist", assetID);
            System.out.println(errorMessage);
            throw new ChaincodeException(errorMessage, AssetTransferErrors.ASSET_NOT_FOUND.toString());
        }

        // If exist read the asset
        Asset asset = genson.deserialize(assetJSON, Asset.class);

        // Generate a unique id for the duplicate asset
        ...
        while (!(duplicateAssetJSON == null || duplicateAssetJSON.isEmpty())) {
            duplicateAssetID = "duplicate-" + assetID + n;
            n++;
        }

        // Create a new duplicate asset
        ...
        // Use a Genson to convert the Asset into string, sort it alphabetically and
        // serialize it into a json string
